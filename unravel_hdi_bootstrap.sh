...
" > /tmp/unravel/final_check.py
    ( sudo nohup python /tmp/unravel/final_check.py -host ${UNRAVEL_HOST} -l ${AMBARI_HOST} -user ${AMBARI_USR} -pass ${AMBARI_PWD} -c ${CLUSTER_ID} -s ${SPARK_VER_XYZ} > /tmp/unravel/final_check.log &) | sudo crontab -
}
